var bg = {
    ...
    resetSubMenu: function(tab) {
        ...
        function addMenu(title, closeIf) {
            bg.menus.push(chrome.contextMenus.create({
                ...
                "onclick": function(info, curTab) {
                    filterTabs(function(tabs, i) {
                        return closeIf(tabs, i, curTab);
                    });
                },
            }));
        }
        ...
        addMenu("Left tabs",
            function(tabs, i, curTab) {
                return tabs[i].index < curTab.index;
            });
        addMenu("Right tabs",
            function(tabs, i, curTab) {
                return tabs[i].index > curTab.index;
        ...
        addMenu("From domain: " + domain,
            function(tabs, i, curTab) {
                return getDomain(tabs[i].url) === getDomain(curTab.url);
            });
        ...
        for (i = urlPartsSkip; i < parts.length; i++) {
            ...
            function subUrl(tab, lastPart) {
                return urlParts(tab.url)
                    .slice(urlPartsSkip, lastPart)
                    .join('');
            }
            let index = i;
            addMenu(
